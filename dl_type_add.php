...
if($type == 'delete'){
$del = mysqli_query($conn, "DELETE FROM `dl_type` WHERE `id`='$id'");
if($del){
    echo "<script>alert('Data Deleted Successfully');window.location='dl_type.php';</script>";
}else{
...
if(isset($_POST['submit_sender']))
{
    ...
    $name=$_POST['name'];
        ...
        if ($id == '') {
            mysqli_query($conn, "insert into `dl_type` (`name`) values ('$name')");
            $id = mysqli_insert_id($conn);
        } else {
            mysqli_query($conn, "update `dl_type` set `name`='$name' where id='$id'");
        }

        echo '<script>alert("Data updated successfully"); window.location="dl_type.php";</script>';
}

if(isset($_GET['id']))
{
    $uid=$_GET['id'];
    $data=mysqli_fetch_assoc(mysqli_query($conn,"select * from `dl_type` where id='$uid'"));
}
}
?>
